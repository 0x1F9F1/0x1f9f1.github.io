...
But what if the hash function is invertible, such that given `hash_of_string = hash(string, seed)`, you can calculate `seed = inv_hash(string, hash_of_string)`?  
...
`x = (1 << n) + 1` is an odd number, and all odd numbers have a [modular multiplicative inverse](https://en.wikipedia.org/wiki/Modular_multiplicative_inverse) mod 2<sup>32</sup>.  
The inverse can be calculated in Python using `y = pow(x, -1, 2**32)`.
...
This expands/simplifies `hash ^= (hash >> (2*n))`.  
...
Using these functions, we can calculate `hash = joaat(key, length, seed)` and `seed = inv_joaat(key, length, hash)`
...
Going forward, `hash = joaat_loop` and `inv_hash = inv_joaat_loop`.  
...
`hash_of_foobar = hash("foobar", 6, seed)` can be split into:  
`hash_of_foo = hash("foo", 3, seed)` and `hash_of_foobar = hash("bar", 3, hash_of_foo)`  

And the same works in reverse.  
`seed = inv_hash("foobar", 6, hash_of_foobar)` can be split into:  
`hash_of_foo = inv_hash("bar", 3, hash_of_foobar)` and `seed = inv_hash("foo", 3, hash_of_foo)`
...
```c
uint32_t hashes_0[3] = {
    0x9290584E, // = hash("foo", 3, seed) | foo
    0x9B85A7ED, // = hash("bar", 3, seed) | bar
    0x9B85C665, // = hash("baz", 3, seed) | baz
};
```
...
uint32_t hashes_1[3 * 3] = {
    0x326FF91C, // = hash("spam", 4, hashes_0[0]) | foospam
    0xA2D2706D, // = hash("spam", 4, hashes_0[1]) | barspam
    0x5E82F68E, // = hash("spam", 4, hashes_0[2]) | bazspam
    0x50AD347B, // = hash("ham",  3, hashes_0[0]) | fooham
    0xFFCD395B, // = hash("ham",  3, hashes_0[1]) | barham
    0x4390BB7E, // = hash("ham",  3, hashes_0[2]) | bazham
    0x9B900B39, // = hash("eggs", 4, hashes_0[0]) | fooeggs
    0x76AB7F74, // = hash("eggs", 4, hashes_0[1]) | bareggs
    0x61E13087, // = hash("eggs", 4, hashes_0[2]) | bazeggs
};
```
...
uint32_t hashes_3[10] = {
    0xBCFCCF1D, // = inv_hash("0", 1, target_hash) | 0
    0xBCFCCF1C, // = inv_hash("1", 1, target_hash) | 1
    0xBCFCCF1B, // = inv_hash("2", 1, target_hash) | 2
    0xBCFCCF1A, // = inv_hash("3", 1, target_hash) | 3
    0xBCFCCF19, // = inv_hash("4", 1, target_hash) | 4
    0xBCFCCF18, // = inv_hash("5", 1, target_hash) | 5
    0xBCFCCF17, // = inv_hash("6", 1, target_hash) | 6
    0xBCFCCF16, // = inv_hash("7", 1, target_hash) | 7
    0xBCFCCF15, // = inv_hash("8", 1, target_hash) | 8
    0xBCFCCF14, // = inv_hash("9", 1, target_hash) | 9
};
```
...
uint32_t hashes_2[3 * 10] = {
    0x44B7CA4F, // = inv_hash("alpha", 5, hashes_3[0]) | alpha0
    0xD5DC41E6, // = inv_hash("alpha", 5, hashes_3[1]) | alpha1
    0xF0724F33, // = inv_hash("alpha", 5, hashes_3[2]) | alpha2
    0x92478EFC, // = inv_hash("alpha", 5, hashes_3[3]) | alpha3
    0x441F4B0B, // = inv_hash("alpha", 5, hashes_3[4]) | alpha4
    0x1E505441, // = inv_hash("alpha", 5, hashes_3[5]) | alpha5
    0x5E82F68E, // = inv_hash("alpha", 5, hashes_3[6]) | alpha6
    0x10DDE129, // = inv_hash("alpha", 5, hashes_3[7]) | alpha7
    0xDC9517D6, // = inv_hash("alpha", 5, hashes_3[8]) | alpha8
    0x68A87890, // = inv_hash("alpha", 5, hashes_3[9]) | alpha9
    0x02645373, // = inv_hash("beta", 4, hashes_3[0]) | beta0
    0x79F25E08, // = inv_hash("beta", 4, hashes_3[1]) | beta1
    0x3BF5E6E1, // = inv_hash("beta", 4, hashes_3[2]) | beta2
    0xAD889649, // = inv_hash("beta", 4, hashes_3[3]) | beta3
    0x78AC4C24, // = inv_hash("beta", 4, hashes_3[4]) | beta4
    0x679BF0A5, // = inv_hash("beta", 4, hashes_3[5]) | beta5
    0x98FFAF13, // = inv_hash("beta", 4, hashes_3[6]) | beta6
    0x329BD300, // = inv_hash("beta", 4, hashes_3[7]) | beta7
    0x69A818A3, // = inv_hash("beta", 4, hashes_3[8]) | beta8
    0x0CA5BAB8, // = inv_hash("beta", 4, hashes_3[9]) | beta9
    0x7E61C3A2, // = inv_hash("gamma", 5, hashes_3[0]) | gamma0
    0x58CCF7F6, // = inv_hash("gamma", 5, hashes_3[1]) | gamma1
    0x2E9CABEE, // = inv_hash("gamma", 5, hashes_3[2]) | gamma2
    0x8454125A, // = inv_hash("gamma", 5, hashes_3[3]) | gamma3
    0x236DF238, // = inv_hash("gamma", 5, hashes_3[4]) | gamma4
    0x81606B74, // = inv_hash("gamma", 5, hashes_3[5]) | gamma5
    0x8FBDD74C, // = inv_hash("gamma", 5, hashes_3[6]) | gamma6
    0x2F4F9E57, // = inv_hash("gamma", 5, hashes_3[7]) | gamma7
    0x71FE275A, // = inv_hash("gamma", 5, hashes_3[8]) | gamma8
    0xD3A812AF, // = inv_hash("gamma", 5, hashes_3[9]) | gamma9
};
```
